...
JNICALL
Java_signal_JNISignal_getSignal(
        JNIEnv *env,
    ...
    //获取包名
    char *pkg_name = jstringTostring(env, getPackageName(env));

    // 获得应用包的信息
    jobject package_info = env->CallObjectMethod(package_manager,
                                                 package_info_id,
                                                 env->NewStringUTF(pkg_name), 64);
    ...
    long long xFactor = 1;
    long long time = ((xFactor * xTime.tv_sec * 1000) + (xTime.tv_usec / 1000)) / 1000 / 60;

    // 拼接 签名+包名+时间
    int len = 25 + static_cast<int>(strlen(pkg_name));
//    char str[len];
    char *str = new char[len];
    sprintf(str, "%d%s%lld", hash_code, pkg_name, time);
    //md5加密
    return env->NewStringUTF(MD5(str).hexdigest().c_str());
}
